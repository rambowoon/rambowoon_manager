...
if (file_exists(pp('src/Routes/web.php')) && file_exists(ap('src/Routes/web.php'))) {
    ...
    if (strpos($webRoutes, 'amp.slugweb') === false && strpos($webRoutes, 'amp.home') === false) {
        ...
        // Inject after "use NASANICORE\Core\Routing\NASANIRouter;" line
        $useStatement = 'use NASANICORE\Core\Routing\NASANIRouter;';
        if (strpos($webRoutes, $useStatement) !== false) {
            ...
            // Fallback: inject after <?php if use statement not found
...
if (file_exists(pp('src/Controllers/Web/ApiController.php')) && file_exists(ap('src/Controllers/Web/ApiController.php'))) {
    ...
    if (strpos($controllerContent, 'return match ($method)') === false && strpos($controllerContent, 'return match($method)') === false) {
        ...
        amp_log("-> Added return to match block in ApiController.php");
    }

    // 8b. Inject 'newsletter-post-amp' case into the match block
    if (strpos($controllerContent, 'newsletter-post-amp') === false) {
        // Find the first case line in the match block and prepend before it
        $controllerContent = preg_replace(
            "/(match\s*\(\\\$method\)\s*\{)\s*\n(\s+)/",
            "$1\n$2'newsletter-post-amp' => \$this->NewsletterPostAMP(\$request),\n$2",
            $controllerContent
        );
        amp_log("-> Injected 'newsletter-post-amp' case into match block");
    } else {
        amp_log("-> 'newsletter-post-amp' case already present");
    }

    // 8c. Append NewsletterPostAMP method before last closing brace of class
    if (strpos($controllerContent, 'function NewsletterPostAMP') === false) {
        $ampController = file_get_contents(ap('src/Controllers/Web/ApiController.php'));

        // Find start of NewsletterPostAMP method
        $methodStart = strpos($ampController, 'public function NewsletterPostAMP');
        if ($methodStart !== false) {
            // Find the opening brace of the method
            $openBrace = strpos($ampController, '{', $methodStart);
            if ($openBrace !== false) {
                // Count braces to find the matching closing brace
                $depth = 0;
                $pos = $openBrace;
                $len = strlen($ampController);
                while ($pos < $len) {
                    if ($ampController[$pos] === '{') $depth++;
                    elseif ($ampController[$pos] === '}') {
                        $depth--;
                        if ($depth === 0) { $methodEnd = $pos; break; }
                    }
                    $pos++;
                }
                if (isset($methodEnd)) {
                    $ampMethod = "\n\n    " . trim(substr($ampController, $methodStart, $methodEnd - $methodStart + 1));
                    // Insert before last closing brace of class
                    $classEnd = strrpos($controllerContent, '}');
                    if ($classEnd !== false) {
                        $controllerContent = substr_replace($controllerContent, $ampMethod . "\n", $classEnd, 0);
                    }
                    amp_log("-> Appended NewsletterPostAMP() method to ApiController.php");
                } else {
                    amp_log("-> Could not find closing brace of NewsletterPostAMP");
                }
            }
        } else {
            amp_log("-> NewsletterPostAMP not found in AMP source");
        }
    } else {
        amp_log("-> NewsletterPostAMP() already exists in ApiController.php");
    }
